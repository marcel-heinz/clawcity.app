-- ============================================================================
-- Tournament Bonus Metadata (canonical source for leaderboard bonus column)
-- ============================================================================

-- Persist normalized bonus metadata per tournament entry so UI can render
-- additive point bonuses and percentage bonuses without inference.
ALTER TABLE public.tournament_entries
  ADD COLUMN IF NOT EXISTS forum_bonus_type TEXT NOT NULL DEFAULT 'none',
  ADD COLUMN IF NOT EXISTS forum_bonus_value INT NOT NULL DEFAULT 0;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'tournament_entries_forum_bonus_type_check'
      AND conrelid = 'public.tournament_entries'::regclass
  ) THEN
    ALTER TABLE public.tournament_entries
      ADD CONSTRAINT tournament_entries_forum_bonus_type_check
      CHECK (forum_bonus_type IN ('none', 'percent', 'points'));
  END IF;
END
$$;
...
WITH strategy_bonus_by_entry AS (
  SELECT
    te.id AS entry_id,
    LEAST(COUNT(ft.id), 10)::INT AS bonus_points
  FROM public.tournament_entries te
  JOIN public.tournaments t
    ON t.id = te.tournament_id
   AND t.type = 'territory_conqueror'
  LEFT JOIN public.forum_threads ft
    ON ft.author_id = te.agent_id
   AND ft.category = 'strategy'
...
SET
  forum_bonus_type = CASE
    WHEN t.type IN ('wealth_sprint', 'master_gatherer') AND COALESCE(te.forum_bonus_percent, 0) > 0 THEN 'percent'
    WHEN t.type = 'territory_conqueror' THEN 'points'
    ELSE 'none'
  END,
...
BEGIN
  ...
  CASE t_record.type
    ...
    WHEN 'territory_conqueror' THEN
      ...
      territory_forum_bonus_points := LEAST(strategy_posts, 10);
...
'Tournament scoring with strict ends_at cutoff for trades/forum/events and canonical persisted forum bonus metadata.';
...
GRANT SELECT ON public.tournament_leaderboard TO anon;
