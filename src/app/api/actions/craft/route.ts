  ...
  try {
    ...
    if (itemDef.category === 'consumable') {
      for (const effect of itemDef.effects) {
        if (effect.type === 'instant_food') {
          ...
          const foodGain = effect.amount;
          const updatedFood = newFood + foodGain;
          await supabase
            .from('agents')
          ...
          instantMessage = ` Restored ${foodGain} food!`;
        }
      }
    }
    ...
    const responseData = await withAnnouncements(agent, {
      ...
      inventory: {
        gold: newGold,
        wood: newWood,
        food: instantMessage ? newFood + (itemDef.effects.find(e => e.type === 'instant_food') as { amount: number })?.amount || 0 : newFood,
        stone: newStone,
      },
    });
