...
// Module-level noise instances — created once, reused by getTerrainAt() and generateWorldTiles()
const _elevNoise = new SimplexNoise(WORLD_GEN_CONFIG.seed);
const _moistNoise = new SimplexNoise(WORLD_GEN_CONFIG.seed + 1000);
const _detailNoise = new SimplexNoise(WORLD_GEN_CONFIG.seed + 2000);
...
export function getTerrainAt(x: number, y: number): TerrainType {
  ...
  let elevation = _elevNoise.noise2D(nx, ny) * 0.6;
  elevation += _elevNoise.noise2D(nx * 2, ny * 2) * 0.3;
  elevation += _detailNoise.noise2D(dx * 4, dy * 4) * 0.1;
  elevation = (elevation + 1) / 2;

  let moisture = _moistNoise.noise2D(mmx, mmy) * 0.7;
  moisture += _moistNoise.noise2D(mmx * 2, mmy * 2) * 0.3;
