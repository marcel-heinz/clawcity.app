...
interface WorldNoiseBundle {
  elevNoise: SimplexNoise;
...
// Market locations: 25 markets in a 5x5 grid pattern across the 500x500 world
...
function worldConfigCacheKey(config: WorldGenConfig): string {
  return `${config.seed}:${config.elevationScale}:${config.moistureScale}:${config.detailScale}`;
}
...
export function normalizeWorldGenConfig(
  config?: Partial<WorldGenConfig> | null
): WorldGenConfig {
  return {
    seed: config?.seed ?? DEFAULT_WORLD_GEN_CONFIG.seed,
    elevationScale: config?.elevationScale ?? DEFAULT_WORLD_GEN_CONFIG.elevationScale,
    moistureScale: config?.moistureScale ?? DEFAULT_WORLD_GEN_CONFIG.moistureScale,
    detailScale: config?.detailScale ?? DEFAULT_WORLD_GEN_CONFIG.detailScale,
  };
}

function resolveTerrainWithBundle(
  x: number,
  y: number,
  config: WorldGenConfig,
  bundle: WorldNoiseBundle
): TerrainType {
  const key = `${x},${y}`;
  if (_marketLocations.has(key)) return 'market';

  const { elevationScale, moistureScale, detailScale } = config;
  const { elevNoise, moistNoise, detailNoise } = bundle;
  ...
  return getBiomeTerrain(elevation, moisture);
}
...
export function createTerrainResolver(
  ...
  return (x: number, y: number) => resolveTerrainWithBundle(x, y, config, bundle);
}

/**
 * Deterministically compute the terrain type at any (x, y) coordinate for a given config.
 */
export function getTerrainAtWithConfig(
  x: number,
  y: number,
  worldConfig?: Partial<WorldGenConfig> | null
): TerrainType {
  const config = normalizeWorldGenConfig(worldConfig);
  const bundle = getNoiseBundle(config);
  return resolveTerrainWithBundle(x, y, config, bundle);
}

/**
 * Deterministically compute terrain using the default active config.
 */
export function getTerrainAt(x: number, y: number): TerrainType {
  return getTerrainAtWithConfig(x, y, DEFAULT_WORLD_GEN_CONFIG);
}
 ...
 */
export function generateWorldTilesWithConfig(
  worldConfig?: Partial<WorldGenConfig> | null
): Array<{ x: number; y: number; terrain: TerrainType; resources: Record<string, number> }> {
  const config = normalizeWorldGenConfig(worldConfig);
  ...
  for (let y = 0; y < WORLD_SIZE; y++) {
    for (let x = 0; x < WORLD_SIZE; x++) {
      tiles.push({
        x,
        y,
        terrain: getTerrainAtWithConfig(x, y, config),
        resources: {},
      });
    }
  }
...
/**
 * Generate a vertical chunk of world tiles [startY, endYExclusive).
 */
export function generateWorldTilesRange(
  startY: number,
  ...
  const config = normalizeWorldGenConfig(worldConfig);
  const tiles: Array<{ x: number; y: number; terrain: TerrainType; resources: Record<string, number> }> = [];
  const clampedStart = Math.max(0, Math.min(WORLD_SIZE, Math.floor(startY)));
  const clampedEnd = Math.max(clampedStart, Math.min(WORLD_SIZE, Math.floor(endYExclusive)));
...
/**
 * Backward-compatible full-world generator with default config.
 */
export function generateWorldTiles(): Array<{ x: number; y: number; terrain: TerrainType; resources: Record<string, number> }> {
  return generateWorldTilesWithConfig(DEFAULT_WORLD_GEN_CONFIG);
}
