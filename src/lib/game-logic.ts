...
const noiseCache = new Map<number, NoiseBundle>();
const marketCache = new Map<number, Set<string>>();

function getNoiseBundle(seed: number): NoiseBundle {
  const cached = noiseCache.get(seed);
  if (cached) return cached;

  const bundle: NoiseBundle = {
    elevNoise: new SimplexNoise(seed),
    moistNoise: new SimplexNoise(seed + 1000),
    detailNoise: new SimplexNoise(seed + 2000),
  };

  noiseCache.set(seed, bundle);
  return bundle;
}

function getMarketLocations(worldSize: number): Set<string> {
  const cached = marketCache.get(worldSize);
  if (cached) return cached;

  // Keep the existing 5x5 market spacing pattern anchored to the same cadence.
  const locations = new Set<string>();
  const spacing = 100;
  const start = 50;
  for (let mx = start; mx < worldSize; mx += spacing) {
    for (let my = start; my < worldSize; my += spacing) {
      locations.add(`${mx},${my}`);
    }
  }

  marketCache.set(worldSize, locations);
  return locations;
}

/**
 * Deterministically compute the terrain type at any (x, y) coordinate.
 * Uses simplex noise with seed 42 — identical to the terrain generated by generateWorldTiles().
 */
export function getTerrainAt(x: number, y: number): TerrainType {
  return getTerrainAtWithSeed(x, y, WORLD_GEN_CONFIG.seed, WORLD_SIZE);
}

/**
 * Deterministically compute the terrain type at any (x, y) coordinate for a specific seed/world size.
 */
export function getTerrainAtWithSeed(
  x: number,
  y: number,
  seed: number,
  ...
  const key = `${x},${y}`;
  const markets = getMarketLocations(worldSize);
  if (markets.has(key)) return 'market';

  const { elevationScale, moistureScale, detailScale } = WORLD_GEN_CONFIG;
  ...
  const { elevNoise, moistNoise, detailNoise } = getNoiseBundle(seed);
...
export function generateWorldTiles(): Array<{ x: number; y: number; terrain: TerrainType; resources: Record<string, number> }> {
  return generateWorldTilesWithSeed(WORLD_GEN_CONFIG.seed, WORLD_SIZE);
}

export function generateWorldTilesWithSeed(
  seed: number,
  worldSize: number = WORLD_SIZE
  ...
  for (let y = 0; y < worldSize; y++) {
    for (let x = 0; x < worldSize; x++) {
      tiles.push({
        x,
        y,
        terrain: getTerrainAtWithSeed(x, y, seed, worldSize),
        resources: {},
      });
    }
  }
