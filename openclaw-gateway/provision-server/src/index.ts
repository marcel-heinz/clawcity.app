...
const hardStoppedAgents = new Set<string>();
const activeGatewayRequests = new Map<string, Set<AbortController>>();
const stopAbortedControllers = new WeakSet<AbortController>();
...
app.delete('/api/provision/:agentId', async (req, res) => {
  try {
    ...
    if (!hardStopConfirmed) {
      ...
      console.error(
        `[provision] Hard-stop verification failed for ${agentId} (not_configured=${verifiedNotConfigured}, drain=${drainVerified})`
      );
      res.status(500).json({
        success: false,
        agentId,
    ...
    console.log(
      `[provision] Agent ${agentId} hard-stopped (removed=${removedFromConfig}, in_flight=${inFlightAtStop}, aborted=${abortedRequests})`
    );
    res.json({
      success: true,
      agentId,
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const callBudget = await consumeCallBudget(
      agentId,
      'manual',
...
function createAgentStoppedError(agentId: string): Error & { code: 'agent_stopped'; agentId: string } {
  const error = new Error(`Agent ${agentId} is hard-stopped. Redeploy to resume.`) as Error & {
    code: 'agent_stopped';
    agentId: string;
  };
...
function isAgentStoppedError(error: unknown): error is Error & { code: 'agent_stopped'; agentId?: string } {
  return Boolean(
    error &&
    typeof error === 'object' &&
    'code' in error &&
    (error as { code?: string }).code === 'agent_stopped'
  );
}

function isAgentHardStopped(agentId: string): boolean {
  return hardStoppedAgents.has(agentId);
}

function markAgentHardStopped(agentId: string): void {
  hardStoppedAgents.add(agentId);
}

function clearAgentHardStopped(agentId: string): void {
  hardStoppedAgents.delete(agentId);
}

function registerGatewayRequest(agentId: string, controller: AbortController): void {
...
function unregisterGatewayRequest(agentId: string, controller: AbortController): void {
  const existing = activeGatewayRequests.get(agentId);
  if (!existing) return;
...
function abortAgentGatewayRequests(agentId: string): number {
  const existing = activeGatewayRequests.get(agentId);
  if (!existing || existing.size === 0) return 0;
...
async function waitForAgentDrain(agentId: string, timeoutMs: number): Promise<boolean> {
...
async function runAutoplayForAgent(agentId: string, options: RunAutoplayOptions = {}): Promise<RunAutoplayResult> {
  ...
  try {
    const budget = await consumeCallBudget(
      agentId,
      'autoplay',
